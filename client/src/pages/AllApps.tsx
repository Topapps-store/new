      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold mb-2">
            {/* Mostrar texto directamente en lugar de usar la clave de traducción */}
            {t('nav.language') === 'es' ? 'Todas las Aplicaciones' : 'All Applications'}
          </h1>
          <p className="text-gray-600">
            {t('nav.language') === 'es' 
              ? 'Descubre y descarga nuestra colección completa de aplicaciones' 
              : 'Discover and download our complete collection of apps'}
          </p>
        </div>
        <button 
          onClick={() => setLocation('/')}
          ...
          {apps && apps.length === 0 && (
            <div className="text-center py-10">
              ...
              <h3 className="text-xl font-semibold mb-2">
                {t('nav.language') === 'es' ? 'No Se Encontraron Aplicaciones' : 'No Applications Found'}
              </h3>
              <p className="text-gray-600">
                {t('nav.language') === 'es' 
                  ? 'Intenta una búsqueda diferente o navega por categoría' 
